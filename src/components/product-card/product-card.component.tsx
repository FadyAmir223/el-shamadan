...
const ProductCard = ({ product }: { product: waferProduct }) => {
  return (
    <Link to={`/products/${product.id}`} className="relative group">
      ...
      />
      <div className="bg-[#181A1B] rounded-lg p-4 md:p-6 lg:p-8 relative before:absolute before:left-0 before:top-0 before:bg-purple before:duration-500 before:h-full before:w-0 hover:before:w-1/2 group before:rounded-r-full mt-16 border-t-8 border-l-8 border-t-yellow border-l-yellow">
